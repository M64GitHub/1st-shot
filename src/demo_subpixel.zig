...
pub fn main() !void {
    ...
    var sprite_left = try Sprite.initFromPng(allocator, "assets/asteroid_big.png", "asteroid_left");
    defer sprite_left.deinit(allocator);

    var sprite_right = try Sprite.initFromPng(allocator, "assets/asteroid_big.png", "asteroid_right");
    ...
    while (running) {
                    ...
                    switch (key.type) {
                        ...
                        .Char => {
                            if (key.sequence.len == 1) {
                                ...
                                } else if (key.sequence[0] == 'y' or key.sequence[0] == 'Y') {
        ...
        if (inner_loop % 200 == 0) {
            ...
            if (show_left) {
                try screen.addRenderSurface(try left_obstacle.sprite.getCurrentFrameSurface());
            }

            if (show_right) {
                try screen.addRenderSurface(try right_obstacle.sprite.getCurrentFrameSurface());
            }
            ...
            _ = screen.output_surface.putStrXY("CONTROLS:", 2, instructions_y, movy.color.WHITE, movy.color.BLACK);
            _ = screen.output_surface.putStrXY("ESC=Quit  UP/DOWN=Rough  LEFT/RIGHT=Smooth  1/2=Toggle  Y=Sync", 2, instructions_y + 1, movy.color.WHITE, movy.color.BLACK);

            // Draw status
            var buf: [80]u8 = undefined;
            const status = try std.fmt.bufPrint(&buf, "Rough Threshold: {d:3}  Smooth Adder: {d:3}", .{
                left_obstacle.frame_threshold,
                right_obstacle.speed_adder,
            });
            _ = screen.output_surface.putStrXY(status, 2, instructions_y - 2, movy.color.WHITE, movy.color.BLACK);

            // Draw smooth frame calculation (only if smooth is visible)
            if (show_right) {
                const f_value: f32 = @as(f32, @floatFromInt(right_obstacle.speed_threshold)) / @as(f32, @floatFromInt(right_obstacle.speed_adder));
                var f_buf: [40]u8 = undefined;
                const f_status = try std.fmt.bufPrint(&f_buf, "F = {d:.2}", .{f_value});
                _ = screen.output_surface.putStrXY(f_status, 50, instructions_y - 2, movy.color.WHITE, movy.color.BLACK);
            }

            // Draw labels above obstacles (only if visible)
            if (show_left) {
                _ = screen.output_surface.putStrXY("ROUGH", @intCast(left_obstacle.x - 2), 1, movy.color.WHITE, movy.color.BLACK);
            }
            if (show_right) {
                _ = screen.output_surface.putStrXY("SMOOTH", @intCast(right_obstacle.x - 2), 1, movy.color.WHITE, movy.color.BLACK);
            }
